...
- **Runtime**: 1 ms
- **Memory**: 17.97 MB
- **Solution**: We need the top k values, in their original order. So sort by value, holding onto the indices, then take the top k, and sort again by the indices. We can actually do this in a 1-liner.
...
- **Runtime**: 830 ms
- **Memory**: 21.24 MB
- **Solution**: We can keep track of how many of each color is on odd or even spaces within the window and if their totals make the complete window alternating.
...
- **Runtime**: 18 ms
- **Memory**: 17.64 MB
- **Solution**: We can simply brute force by taking each fruit and going through baskets looking for a place for it. If we find one, we can set that baskets capacity to now 0. Count up leftover baskets.
